# UNIT I: FREE LEARNING RESOURCES

## CHAPTER 2: UNDERSTANDING ODOO

These resources become **strongly Odoo-specific**: editions, hosting, apps/modules, users, companies, shared records, Studio, and customization from [Chapter 2 Content](Content.md).

Use them to see the current product generation, not only to read concepts.

Primary sources support the key concepts in this chapter. Videos and repository listings are supplementary resources and may change over time. Odoo-specific references use version 19.0 unless stated.

---

## CHAPTER 2 RESOURCES TABLE OF CONTENTS

- [YouTube](#youtube)
- [Official Odoo Learning Website](#official-odoo-learning-website)
- [Official Documentation](#official-documentation)
- [Repositories](#repositories)
- [Practice / Lab Environments](#practice--lab-environments)
- [Recommended Study Order](#recommended-study-order)

---

## YOUTUBE

> **How previews work on GitHub:** Click a thumbnail to open the video on YouTube. GitHub Markdown cannot embed an inline player, but thumbnails give you a visual preview without leaving the page layout.

### 1. ODOO BEGINNER'S GUIDE

| | |
|---|---|
| **Relevant to** | [2.1 What Odoo Is](Content.md#21-what-odoo-is), [2.8 Odoo Apps](Content.md#28-odoo-apps), [2.2 Odoo Ecosystem](Content.md#22-odoo-ecosystem) |
| **Source** | Community tutorial |
| **Why use it** | Good broad introduction before studying individual applications |

<div align="center">

[![Odoo Beginner's Guide](https://img.youtube.com/vi/QuC6rc2q2mg/hqdefault.jpg)](https://www.youtube.com/watch?v=QuC6rc2q2mg)

**Watch on YouTube:** [Odoo Beginner's Guide](https://www.youtube.com/watch?v=QuC6rc2q2mg)

</div>
---

### 2. MEET ODOO 19'S BEST FEATURES

| | |
|---|---|
| **Relevant to** | [2.1 What Odoo Is](Content.md#21-what-odoo-is), [2.4 Enterprise Edition](Content.md#24-enterprise-edition), [2.8 Odoo Apps](Content.md#28-odoo-apps) |
| **Source** | Official Odoo YouTube channel |
| **Why use it** | Useful for seeing the current generation of Odoo rather than only reading concepts |

<div align="center">

[![Meet Odoo 19's Best Features](https://img.youtube.com/vi/OZLP-SCHW7A/hqdefault.jpg)](https://www.youtube.com/watch?v=OZLP-SCHW7A)

**Watch on YouTube:** [Meet Odoo 19's Best Features](https://www.youtube.com/watch?v=OZLP-SCHW7A)

</div>
---

### 3. ODOO STUDIO: BUILD AND CUSTOMIZE APPS

| | |
|---|---|
| **Relevant to** | [2.14 Odoo Studio Concept](Content.md#214-odoo-studio-concept), [2.13 Standard vs Custom Modules](Content.md#213-standard-vs-custom-modules) |
| **Source** | Official Odoo YouTube channel |
| **Why use it** | Directly relevant to understanding no-code customization inside Odoo |

<div align="center">

[![Odoo Studio: Build and Customize Apps](https://img.youtube.com/vi/IRbj-SzTcrY/hqdefault.jpg)](https://www.youtube.com/watch?v=IRbj-SzTcrY)

**Watch on YouTube:** [Odoo Studio: Build and Customize Apps](https://www.youtube.com/watch?v=IRbj-SzTcrY)

</div>
---

### 4. ODOO ONLINE VS ODOO.SH VS ON-PREMISE

| | |
|---|---|
| **Relevant to** | [2.5 Odoo Online](Content.md#25-odoo-online), [2.6 Odoo.sh](Content.md#26-odoosh), [2.7 On-Premise Odoo](Content.md#27-on-premise-odoo) |
| **Source** | Hosting comparison tutorial |
| **Why use it** | Reinforces that **Online ≠ Odoo.sh ≠ On-Premise** |

<div align="center">

[![Odoo Online vs Odoo.sh vs On-Premise](https://img.youtube.com/vi/ws3Z7fhhxuA/hqdefault.jpg)](https://www.youtube.com/watch?v=ws3Z7fhhxuA)

**Watch on YouTube:** [Odoo Online vs Odoo.sh vs On-Premise](https://www.youtube.com/watch?v=ws3Z7fhhxuA)

</div>
---

### 5. WORKING WITH ODOO.SH

| | |
|---|---|
| **Relevant to** | [2.6 Odoo.sh](Content.md#26-odoosh), deployment concepts |
| **Source** | Community / partner tutorial |
| **Why use it** | Shows projects, custom code, staging branches, backups, and deployment workflow |

<div align="center">

[![Working with Odoo.sh](https://img.youtube.com/vi/vsqzfA3ERD8/hqdefault.jpg)](https://www.youtube.com/watch?v=vsqzfA3ERD8)

**Watch on YouTube:** [Working with Odoo.sh](https://www.youtube.com/watch?v=vsqzfA3ERD8)

</div>
---

### 6. IS SELF-HOSTED ODOO RIGHT FOR YOU?

| | |
|---|---|
| **Relevant to** | [2.7 On-Premise Odoo](Content.md#27-on-premise-odoo), [2.3 Community Edition](Content.md#23-community-edition) |
| **Source** | Hosting decision guide |
| **Why use it** | Useful for understanding when self-hosting makes sense |

<div align="center">

[![Is Self-Hosted Odoo Right for You?](https://img.youtube.com/vi/ngbrg7DUOeQ/hqdefault.jpg)](https://www.youtube.com/watch?v=ngbrg7DUOeQ)

**Watch on YouTube:** [Is Self-Hosted Odoo Right for You?](https://www.youtube.com/watch?v=ngbrg7DUOeQ)

</div>
---

## OFFICIAL ODOO LEARNING WEBSITE

### ODOO LEARN (ODOO SLIDES)

| | |
|---|---|
| **Relevant to** | Entire Chapter 2 and the rest of Unit I |
| **Type** | Official free Odoo learning platform |
| **Why use it** | This should become one of your main companion resources throughout the roadmap |

[Open Odoo Learn](https://www.odoo.com/slides)
---

## OFFICIAL DOCUMENTATION

All links below target **Odoo 19.0** documentation unless noted.

### 1. ODOO 19 USER DOCUMENTATION (MAIN ENTRY)

[Online hosting](https://www.odoo.com/documentation/19.0/administration/odoo_online.html) clarifies custom-addon restrictions; the [Odoo.sh FAQ](https://www.odoo.sh/faq) clarifies service eligibility. Use them to defend the hosting choice in the project rather than relying on a broad promotional video.

| | |
|---|---|
| **Relevant to** | Entire Chapter 2 |
| **Type** | Official functional documentation |

[Odoo 19 User Documentation](https://www.odoo.com/documentation/19.0/)

---

### 2. MODELS, MODULES AND APPS

| | |
|---|---|
| **Relevant to** | [2.8 Odoo Apps](Content.md#28-odoo-apps), [2.9 Modules / Addons](Content.md#29-modules--addons) |

[Models, Modules and Apps](https://www.odoo.com/documentation/19.0/applications/general/apps_modules.html)
---

### 3. ODOO STUDIO

| | |
|---|---|
| **Relevant to** | [2.14 Odoo Studio Concept](Content.md#214-odoo-studio-concept) |

[Odoo Studio Documentation](https://www.odoo.com/documentation/19.0/applications/studio.html)

---

### 4. ODOO.SH DOCUMENTATION

| | |
|---|---|
| **Relevant to** | [2.6 Odoo.sh](Content.md#26-odoosh): hosting model, development environments, staging, deployment |

[Odoo.sh Documentation](https://www.odoo.com/documentation/19.0/administration/odoo_sh.html)

[Odoo.sh Getting Started](https://www.odoo.com/documentation/19.0/administration/odoo_sh/getting_started.html)

---

### 5. SELF-HOSTED / SOURCE INSTALLATION

| | |
|---|---|
| **Relevant to** | [2.7 On-Premise Odoo](Content.md#27-on-premise-odoo) |

[Odoo 19 Source Installation](https://www.odoo.com/documentation/19.0/administration/on_premise/source.html)

---

### 6. ODOO ADMINISTRATION

[Multi-company](https://www.odoo.com/documentation/19.0/applications/general/companies/multi_company.html) explains shared contacts/products and company context. Find one common product value and one company-specific value, then explain why the Qatar and Pakistan transactions remain distinct. This is more specific to Sections 2.11–2.12 than the administration landing page.

| | |
|---|---|
| **Relevant to** | Databases, deployment-related topics, multi-company context |

[Odoo 19 Administration](https://www.odoo.com/documentation/19.0/administration.html)
---

## REPOSITORIES

Do **not** try to understand the entire codebase yet. For now, browse structure, application names, and module directories.

### 1. OFFICIAL ODOO SOURCE CODE

Read the [19.0 Sales manifest](https://github.com/odoo/odoo/blob/19.0/addons/sale/__manifest__.py) and identify its name, dependencies, and data files. The [manifest reference](https://www.odoo.com/documentation/19.0/developer/reference/backend/module.html) explains those keys. This is a read-only conceptual exercise; installing or understanding the full source tree is unnecessary here.

| | |
|---|---|
| **Relevant to** | [2.8 Odoo Apps](Content.md#28-odoo-apps), [2.9 Modules / Addons](Content.md#29-modules--addons) |
| **Study branch** | select `19.0` explicitly to match this guide |
| **Browse first** | `addons/` directory: application names and module folders |

[GitHub: odoo/odoo](https://github.com/odoo/odoo)
---

### 2. OFFICIAL ODOO DOCUMENTATION SOURCE

| | |
|---|---|
| **Relevant to** | Understanding how official docs are maintained |
| **Study branch** | use `19.0` when available; default branches can change |

[GitHub: odoo/documentation](https://github.com/odoo/documentation)

---

### 3. OFFICIAL ODOO TUTORIALS REPOSITORY

| | |
|---|---|
| **Relevant to** | Later developer chapters (bookmark now) |
| **Study branch** | use `19.0` when available; default branches can change |
| **Contains** | Code bases and solutions for official Odoo tutorials |

[GitHub: odoo/tutorials](https://github.com/odoo/tutorials)

---

### 4. ODOO TECHNICAL TRAINING

| | |
|---|---|
| **Relevant to** | Later technical units |
| **Why bookmark it** | Official training repository for deeper developer learning |

[GitHub: odoo/technical-training](https://github.com/odoo/technical-training)

---

### 5. ODOO TECHNICAL TRAINING SOLUTIONS

| | |
|---|---|
| **Relevant to** | Later technical units |
| **Why bookmark it** | Solution reference for the official technical training |

[GitHub: odoo/technical-training-solutions](https://github.com/odoo/technical-training-solutions)

---

### 6. AWESOME ODOO

| | |
|---|---|
| **Relevant to** | Discovery index for Odoo resources |
| **Source** | Community-curated list |
| **Note** | Useful for discovery, but this roadmap prioritizes official Odoo sources |

[GitHub: desdelinux/awesome-odoo](https://github.com/desdelinux/awesome-odoo)

---

## PRACTICE / LAB ENVIRONMENTS

### 1. ODOO EDUCATION: FREE PRACTICE DATABASE

| | |
|---|---|
| **Relevant to** | Hands-on exploration of [2.8 Apps](Content.md#28-odoo-apps), [2.10 Users](Content.md#210-users), [2.11 Companies](Content.md#211-companies) |
| **Best for** | Educators and eligible students |
| **Access** | Free educational Odoo Online databases (24-month validity when eligible) |

[Odoo Education: Odoo Online](https://www.odoo.com/education/odoo-online)

[Odoo Education Program](https://www.odoo.com/education/program)

---

### 2. FREE ODOO ONLINE TRIAL

| | |
|---|---|
| **Relevant to** | General hands-on practice |
| **Best for** | Experimentation, not permanent roadmap work |
| **Note** | Temporary environment; no payment needed to create a trial |

[Create an Odoo Trial](https://www.odoo.com/trial)
---

### 3. ODOO RUNBOT

| | |
|---|---|
| **Relevant to** | Inspecting current Odoo builds and official codebase behavior |
| **Best for** | Developer/testing sandbox, not beginner coursework storage |
| **Important** | Builds may reset, change, fail, or disappear |

[Odoo Runbot](https://runbot.odoo.com/)

Treat Runbot as: *"I want to quickly inspect what current Odoo looks like."*

Not as: *"This is where I will store my learning project."*
---

## RECOMMENDED STUDY ORDER

After the overview, write one paragraph selecting hosting for the Bilal scenario, one permission example, and one shared-record example. Use the focused official sources above to resolve a specific uncertainty. A completed watch list without those explanations does not demonstrate Chapter 2 mastery.

If you do not want to consume everything, use this sequence for Chapter 2:

1. **Odoo Beginner's Guide** (YouTube)
2. **Odoo Learn** (official slides platform)
3. **Official Odoo 19 documentation** (apps, modules, hosting)
4. **Browse `odoo/odoo` on GitHub** (look at `addons/` only)

That gives you **product overview + official reference + source-code exposure** before Chapter 3 functional apps.

---

**Previous:** [Chapter 1 Resources](../Chapter%201/Resources.md) | **Next:** [Chapter 3 Resources](../Chapter%203/Resources.md) | **Back to content:** [Chapter 2 Content](Content.md)
